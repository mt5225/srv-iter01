module.exports = 
  APPID : 'wxe2bdce057501817d'
  APPSecret : 'c907a867dc3deebff5c0b2c392c77b90'
  PING_APPID : 'app_rLu1G8LCm9eDL80S'
  PING_TRADE_KEY : 'sk_live_twTLI4mLYMAeSaQ30BwpRHwW'
  PING_TEST_KEY : 'sk_test_TKin5C488mbH88CeTKH40KuT'
  REDIRECT_URL : 'http://qa.aghchina.com.cn:9000'
  MNGT_URL : 'http://qa.aghchina.com.cn:3033'
  MANAGER_OPENID_LIST : ['o82BBs8XqUSk84CNOA3hfQ0kNS90', 'o82BBs9o0ndCJPt0bDxrzhpettpE'] #管家OPEN_ID列表
  NEWRELIC_KEY : 'cae2b2befca3c7197c79de454df83d76c540456d'
  WECHAT_MSG_T : 
    resv_success : 'mP3VycImBswOGo2p7qIS2SvxwiAbXGVwZwNzR5BzmJY' #新订单通知 [to user and manager]
    cancel_warnning : '3X_8Fs8aTn6N1XDw_1UF8yK0ZGJW3V2nQYjdIFG9tE0' #订单无效通知, 用于系统自动取消订单的告警消息 [to user and manager]
    order_cancel : 'h8pLBBplyHr-g2tlw1QKKKU06Cau8tWmetiTR8VrE18' #酒店预订取消成功通知[to user and manager]
    pay_success : 'bevu6h7u6mRdNzzgZA4Yo-UMUF4Dvz8eteGPIiFBlgg' #支付成功提醒 [to user and manager]
    before_checkin : 'fAx0tpF4GHmyJ2nE_8p1iJ1s5dUs6JQHpHc-y6EjmNA' #入住提醒
    order_success : 'uOm5FJoXqvod2MFZfWH0Y0__ZQEZ5vuB4oswxh88hwc' #预订成功通知 [to user and manager]
    mngt_order_cancel: 'NCBC13AbZf_xgiesPJ-YUUvLPbcV776tuCICwFKNPoM' #客人退订通知  [to manager]
    test : 'wVs3fCjtb9kvcnE4vscX64bfjmTi5uLoqt6oPjUWmt0'

###
o82BBs8XqUSk84CNOA3hfQ0kNS90 Jerry
o82BBs9o0ndCJPt0bDxrzhpettpE Luke
###